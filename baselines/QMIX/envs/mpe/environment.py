    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                ...
                if action[0] == 1:
                    agent.action.u[0] = -1.0 * agent.step_size
                if action[0] == 2:
                    agent.action.u[0] = +1.0 * agent.step_size
                if action[0] == 3:
                    agent.action.u[1] = -1.0 * agent.step_size
                if action[0] == 4:
                    agent.action.u[1] = +1.0 * agent.step_size
                d = self.world.dim_p
            else:
                if self.discrete_action_space:
                    agent.action.u[0] += (action[0][1] - action[0][2]) * agent.step_size
                    agent.action.u[1] += (action[0][3] - action[0][4]) * agent.step_size
                    d = 5
                else:
