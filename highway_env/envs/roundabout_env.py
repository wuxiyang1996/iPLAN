...
class RoundaboutEnv(AbstractEnv):
    ...
    def _make_vehicles(self) -> None:
        ...
        vehicle = other_vehicles_type.make_on_lane(self.road,
                                                   ("we", "sx", 1),
                                                   longitudinal=5 + np.random.randn()*position_deviation,
                                                   speed=16 + np.random.randn() * speed_deviation)
        ...
        for i in list(range(1, 2)) + list(range(-1, 0)):
            vehicle = other_vehicles_type.make_on_lane(self.road,
                                                       ("we", "sx", 0),
                                                       longitudinal=20*i + np.random.randn()*position_deviation,
                                                       speed=16 + np.random.randn() * speed_deviation)
        ...
        vehicle = other_vehicles_type.make_on_lane(self.road,
                                                   ("eer", "ees", 0),
                                                   longitudinal=50 + np.random.randn() * position_deviation,
                                                   speed=16 + np.random.randn() * speed_deviation)
        vehicle.plan_route_to(self.np_random.choice(destinations))
        vehicle.randomize_behavior()
        self.road.vehicles.append(vehicle)
